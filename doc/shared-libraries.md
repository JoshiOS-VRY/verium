...
The purpose of this library is to make the verification functionality that is critical to Verium's consensus available to other applications, e.g. to language bindings.
...
`bitcoinconsensus_version` returns an `unsigned int` with the API version _(currently at an experimental `0`)_.
...
- `bitcoinconsensus_error* err` - Will have the error/success code for the operation _(see below)_.
...
- `bitcoinconsensus_ERR_OK` - No errors with input parameters _(see the return value of `bitcoinconsensus_verify_script` for the verification status)_
