...
function formatScanPercent(progress: number | undefined): string {
...
export function LightWalletSyncStatus({
  ...
  const showScanProgress = lightSyncing && scanProgress < 1;
  ...
  if (showScanProgress || balanceSyncing) {
    const phaseLabel = scanPhaseLabel(wallet.light_scan_phase);
    return (
  ...
  if (!lightSyncing && !balanceSyncing && balanceReady) {
    return (
      <p
        className={cn(
  ...
  if (!lightSyncing && !balanceSyncing && !balanceReady) {
    return (
      <p
        className={cn(
          'text-center text-[10px] font-medium uppercase tracking-wide text-fg-muted',
          className
        )}
      >
