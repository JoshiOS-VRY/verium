    ...
    <section className="mobile-panel rounded-2xl border border-border bg-bg-panel/60 p-4">
      ...
      <ul className="mt-3 space-y-2">
        {events.map((ev) => {
          ...
          return (
            <li key={`${ev.address}-${ev.eventType ?? tone}`}>
              <ExplorerCardLink
                target={{ kind: 'address', address: ev.address }}
                ariaLabel={`View address ${ev.address}`}
                className={cn(
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 font-mono text-[11px] text-accent break-all">
