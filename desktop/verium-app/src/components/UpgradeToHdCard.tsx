  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Enable BIP39 recovery phrase support. Back up wallet.dat first — this applies a new HD
          seed via sethdseed.
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        {apply.isSuccess && <p className="mt-2 text-xs text-success">{apply.data}</p>}
        {apply.error && <p className="mt-2 text-xs text-danger">{String(apply.error)}</p>}
      </CardContent>
    </Card>
  );
}
