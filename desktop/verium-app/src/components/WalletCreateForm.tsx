...
export function WalletCreateForm({
  ...
  const disabled = !matches || score.score < 2 || !acknowledged || create.isPending;
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-fg-muted">
            Choose a passphrase to encrypt <span className="font-mono text-xs">wallet.dat</span> and
            unlock the wallet day to day. Vericonomy cannot reset a lost passphrase. On the next
            step you can save a 24-word recovery phrase to restore access if you lose this
            passphrase or your computer — or skip that and rely on a{' '}
      ...
      <label className="flex cursor-pointer items-start gap-2 text-xs text-fg-muted">
        ...
        />
        <span>
          I understand I need this passphrase to unlock and spend. If I skip the recovery phrase
          later, I must back up <span className="font-mono">wallet.dat</span> and keep my passphrase
      ...
      {create.error && (
        <div className="rounded-md border border-danger/30 bg-danger/10 px-3 py-2 text-xs text-danger">
          {String(create.error).toLowerCase().includes('encrypted wallet') ? (
            <>
              This wallet is already encrypted. Use the unlock form with your existing passphrase
