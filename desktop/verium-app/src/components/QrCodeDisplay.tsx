...
export function QrCodeDisplay({
  ...
  const { copied: uriCopied, copy, reset: resetCopyFeedback } = useCopyToClipboard();

  useEffect(() => {
    void buildPaymentUri(coin, address, amount ?? undefined, label, message).then(setUri);
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <p className="max-w-xs break-all text-center text-[10px] text-fg-muted">{uri}</p>
      <div className="flex gap-2">
        <Button
          ...
          className={cn(
            uriCopied && 'border-success/40 bg-success/10 text-success hover:bg-success/15'
          )}
          aria-live="polite"
        >
          {uriCopied ? (
