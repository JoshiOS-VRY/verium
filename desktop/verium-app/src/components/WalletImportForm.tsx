...
import { rpcWalletRestore, tauriWalletFileStatus } from '@/lib/rpc/client';
...
export function WalletImportForm({ onRestored, onCancel, className }: WalletImportFormProps) {
      ...
      <div className={cn('flex flex-col gap-4', className)}>
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-fg-muted">
              Choose a <span className="text-xs">wallet.dat</span> backup from Verium-Qt, this app,
              or another machine. Your current wallet file is saved to backups before it is
              replaced.
            </p>
          </div>
        </div>
        ...
        {!pendingPath ? (
          ...
          <div className="space-y-2 rounded-md border border-warning/40 bg-warning/10 px-3 py-3 text-xs">
            ...
            <p className="break-all text-[11px] text-fg-muted">{pendingPath}</p>
            <p className="text-fg-muted">
              The node restarts and swaps in this backup. Unlock afterward with the passphrase from
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              <Button
                ...
                onClick={() =>
                  void twoFa.gate(
                    'restore_wallet',
                    (code) => restore.mutate({ sourcePath: pendingPath, totpCode: code }),
