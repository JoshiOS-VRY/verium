...
describe('isCoinSetupComplete', () => {
  ...
  it('treats an on-disk light wallet as setup complete', () => {
    expect(isCoinSetupComplete('verium', emptyPrefs, { hasLightWallet: true })).toBe(true);
  });
});
...
describe('fullNodeWalletExists', () => {
  it('detects datadir and legacy Qt wallets', () => {
    expect(fullNodeWalletExists({ exists: true, legacy_wallet_detected: false })).toBe(true);
    expect(fullNodeWalletExists({ exists: false, legacy_wallet_detected: true })).toBe(true);
    expect(fullNodeWalletExists({ exists: false, legacy_wallet_detected: false })).toBe(false);
  });
});
...
function mockProfile(
  ...
  return {
    ...
    ready: false,
    openable: partial.openable ?? (light === true || fullNode === true || legacy === true),
    light_keystore_health: partial.light_keystore_health ?? (light ? 'ok' : 'missing'),
  };
}
...
describe('profileWalletPresence', () => {
  it('derives light and full-node presence from wallet profile', () => {
    expect(
      profileWalletPresence(mockProfile({ light: true, full_node: true, legacy: false }))
    ).toEqual({
      hasLightWallet: true,
      hasFullNodeWallet: true,
    });
    expect(
      profileWalletPresence(mockProfile({ light: false, full_node: false, legacy: true }))
    ).toEqual({
      hasLightWallet: false,
      hasFullNodeWallet: true,
    });
