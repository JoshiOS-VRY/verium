...
export function explorerExtractionHash(coin: CoinId): string {
  return coin === 'verium' ? `${EXPLORER_LINK_BASE}/vrm/miners` : `${chainBase(coin)}/`;
}
...
export function migrateStagingExplorerTemplate(template: string): string {
  ...
  return template.replaceAll('staging-explorer.vericonomy.com', 'explorer.vericonomy.com');
}
...
export function migrateExplorerPrefs(
  ...
  if (nextTx === tx && nextBlock === block && nextAddr === addr) {
    return null;
  }
  return {
    ...(nextTx && nextTx !== tx ? { explorer_tx_url_template: nextTx } : {}),
    ...(nextBlock && nextBlock !== block ? { explorer_block_url_template: nextBlock } : {}),
    ...(nextAddr && nextAddr !== addr ? { explorer_address_url_template: nextAddr } : {}),
  };
}
...
function legacyHashTemplate(coin: CoinId, fragment: 'tx' | 'block' | 'address'): string {
  const host =
    coin === 'verium'
      ? 'https://explorer-vrm.vericonomy.com'
      : 'https://explorer-vrc.vericonomy.com';
...
export function explorerTemplateTargetsOtherChain(coin: CoinId, stored: string): boolean {
...
export function effectiveTxExplorerTemplate(coin: CoinId, stored: string | undefined): string {
  return effectiveExplorerTemplate(coin, stored, 'tx', defaultTxExplorerTemplate);
}

export function effectiveBlockExplorerTemplate(coin: CoinId, stored: string | undefined): string {
  return effectiveExplorerTemplate(coin, stored, 'block', defaultBlockExplorerTemplate);
}

export function effectiveAddressExplorerTemplate(coin: CoinId, stored: string | undefined): string {
  return effectiveExplorerTemplate(coin, stored, 'address', defaultAddressExplorerTemplate);
}
...
export function buildTxExplorerUrl(coin: CoinId, template: string, txid: string): string {
  ...
  const safe = resolved && resolved.includes('%s') ? resolved : defaultTxExplorerTemplate(coin);
...
export function buildBlockExplorerUrl(
  ...
  const safe = resolved && resolved.includes('%s') ? resolved : defaultBlockExplorerTemplate(coin);
  return safe.replace('%s', encodeURIComponent(String(blockHashOrHeight)));
}

export function buildAddressExplorerUrl(coin: CoinId, template: string, address: string): string {
  const resolved = effectiveAddressExplorerTemplate(coin, template);
  const safe =
    resolved && resolved.includes('%s') ? resolved : defaultAddressExplorerTemplate(coin);
