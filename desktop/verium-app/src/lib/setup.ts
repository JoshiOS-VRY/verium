...
export function profileWalletPresenceFromProfile(
  profile: WalletProfile,
): CoinWalletPresence {
  return {
    hasLightWallet: profile.keys_present.light,
    hasFullNodeWallet:
      profile.keys_present.full_node || profile.legacy.detected,
  };
}

/** True when any persisted wallet keys can be opened (decryptable for light). */
export function isProfileOpenable(
...
export function isLightKeystoreUnreadable(
  ...
  return (
    profile?.light_keystore_health === "unreadable" &&
    profile.keys_present.light === true
  );
}
...
export function needsLightWalletRecovery(
  profile: Pick<
    WalletProfile,
    "light_keystore_health" | "keys_present" | "mode" | "ready" | "legacy"
  > | null | undefined,
  walletMode?: WalletModeChoice,
): boolean {
...
export function getHubCardState(options: {
  ...
  const readyFromHub = isCoinWalletReady("verium", { setup_completed: false }, {
    walletMode,
  ...
  const ready =
    readyFromHub ||
    (profile?.ready === true &&
      (profile.mode as WalletModeChoice) === walletMode);
  ...
  if (unreadable) {
    ...
    statusLabel =
      walletMode === "light" ? "Full node on device" : "Light wallet on device";
  } else {
    statusLabel = "Not set up";
  }

  const actionLabel: "Open" | "Set up" | "Import phrase" =
    unreadable ? "Import phrase" : ready || openable ? "Open" : "Set up";
