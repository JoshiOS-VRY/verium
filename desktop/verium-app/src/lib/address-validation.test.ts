...
describe('validateSendAddress', () => {
  ...
  it('rejects wrong length', () => {
    expect(validateSendAddress('VY6E3KSqrMk1hcy5Cu4EGyHrdDS5ch3YH')).toMatch(/exactly 34/);
  });

  it('rejects special characters', () => {
    expect(validateSendAddress(`${VALID.slice(0, 33)}!`)).toMatch(/letters and numbers/);
  });
});
