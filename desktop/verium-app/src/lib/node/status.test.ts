...
describe('node status mapping', () => {
  ...
  it('maps connected sync to ready states', () => {
    expect(isNodeReady({ connected: true, state: 'connected_syncing' } as NodeStatus)).toBe(true);
    expect(isNodeReady({ connected: true, state: 'connected_ready' } as NodeStatus)).toBe(true);
  ...
  it('maps backend state strings', () => {
    expect(nodeStateFromStatus({ state: 'auth_mismatch', connected: false } as NodeStatus)).toBe(
      'auth_mismatch'
    );
  });
