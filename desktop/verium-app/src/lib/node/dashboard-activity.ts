 ...
 */
export function deriveDashboardActivity(input: DashboardActivityInput): DashboardActivity {
  const { coin, status, statusLoading, isConnecting, blockchain, blockchainLoading, networkTip } =
    input;
  ...
  if (isBinaryUnavailableError(status?.error)) {
    return {
      ...
      detail: status?.error ?? 'Install or rebuild the node software, then reopen the wallet.',
      showSpinner: false,
    };
  }
  ...
  if (isConnecting || !status?.connected) {
    ...
    if (status?.warming_up) {
      return {
        ...
        detail: status.user_message ?? status.error ?? 'Your node is reading local block files.',
        showSpinner: true,
        progress: status.verification_progress,
      };
    }
  ...
  if (status.reindex_in_progress) {
    return {
      ...
      showSpinner: true,
      progress: status.verification_progress ?? blockchain?.verificationprogress,
    };
  }
  ...
  if (status.warming_up) {
    return {
      kind: 'warming_up',
      title: `Opening ${name} chain data…`,
      detail: status.user_message ?? status.error ?? 'Almost ready to connect to the network.',
      showSpinner: true,
      progress: status.verification_progress ?? blockchain?.verificationprogress,
    };
  }
  ...
  if (blockchainLoading && !blockchain) {
    return {
      ...
      detail: 'Your node is online — reading the local chain before blocks appear here.',
      showSpinner: true,
      progress: status.verification_progress,
    };
  }
  ...
  const progress = blockchain?.verificationprogress ?? status.verification_progress;
  ...
  if (status.connections === 0 && (blocks ?? 0) < 1_000_000 && phase !== 'synced') {
    return {
      kind: 'finding_peers',
  ...
  if (phase === 'syncing' || blockchain?.initialblockdownload) {
    ...
    return {
      ...
      detail:
        [progressPct && `${progressPct} verified`, behindLine].filter(Boolean).join(' ') ||
...
export function showDashboardActivityBanner(activity: DashboardActivity): boolean {
...
export function heroStatusPillLabel(activity: DashboardActivity, synced: boolean): string {
...
export function nodeStateIndicatesLoading(state: ReturnType<typeof nodeStateFromStatus>): boolean {
  return (
    state === 'starting' ||
    state === 'warming_up' ||
    state === 'reindexing' ||
