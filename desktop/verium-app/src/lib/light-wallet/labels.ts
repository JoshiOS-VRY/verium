...
export function electrumServerShortLabel(
  ...
  if (status.servers_total > 1) {
    return status.failover_index === 0 ? `${symbol} primary` : `${symbol} backup`;
  }
...
export function electrumUriFriendlyLabel(uri: string, coin: CoinId, index: number): string {
  const host = uri
    .replace(/^tls:\/\//i, '')
    .replace(/^ssl:\/\//i, '')
    .replace(/^tcp:\/\//i, '')
...
export function electrumStatusTitle(coin: CoinId, status: LightServerStatus): string {
  const lines = [
    electrumServerLongLabel(coin, status),
    status.tip_height != null ? `Chain height: ${formatChainHeight(status.tip_height)}` : null,
