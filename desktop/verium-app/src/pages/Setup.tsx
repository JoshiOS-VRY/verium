...
export function Setup() {
  ...
  const { isLight, mode: persistedWalletMode, isLoading: walletModeLoading } = useWalletMode();
  ...
  const handleSetupWalletModeChange = useCallback(async (mode: 'light' | 'full_node') => {
              ...
              {setupCoinKeystoreUnreadable ? (
                <>
                  <div className="rounded-md border border-warning/40 bg-warning/10 px-3 py-3 text-xs text-fg-muted">
                    ...
                    <p className="mt-1">
                      Wallet files are on this device but the saved keys need to be rebuilt.
                      Passphrase unlock will not work — import your 24-word recovery phrase (or HD
                      master key) below. Seeds stay encrypted with your new passphrase; no Windows
