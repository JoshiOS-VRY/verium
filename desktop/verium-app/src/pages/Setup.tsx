...
import { useActiveCoin, useEnabledCoins, useSetActiveCoin } from '@/lib/coin/context';
...
import { resolveWalletSetupMode, walletSetupModeLabel } from '@/lib/wallet-setup';
...
import { recoveryApplyHdSeed, recoveryWalletIsHd, twoFactorStatus } from '@/lib/security/client';
...
export function Setup() {
  ...
  const { isPhoneLayout, isTabletLayout } = useResponsiveLayout();
  const invalidateWalletMode = useInvalidateWalletMode();
  const [setupWalletMode, setSetupWalletMode] = useState<'light' | 'full_node'>('full_node');
  ...
  const goToHub = useCallback(() => {
    ...
    void queryClient.invalidateQueries({
      predicate: (q) => Array.isArray(q.queryKey) && q.queryKey[1] === 'wallet-profile',
    });
  ...
  const openReadyCoinDashboard = useCallback(
    ...
    [prefs, updatePrefs, invalidateWalletMode, navigate, queryClient]
  );

  const startCoinSetup = useCallback(
    async (targetCoin: CoinId) => {
      const walletProfile = await tauriWalletProfile(targetCoin).catch(() => null);
  ...
  const startMobileWalletFlow = useCallback(
    async (targetCoin: CoinId, action: 'create' | 'import' | 'open') => {
      setHubOpeningCoin(targetCoin);
      try {
        if (action === 'open') {
          const walletProfile = await tauriWalletProfile(targetCoin).catch(() => null);
    ...
    [openReadyCoinDashboard, resetCoinOnboarding, setActiveCoin, invalidateWalletMode]
  );
  ...
  const walletFile = useQuery({
    ...
    refetchInterval: step === 'daemon' ? 4_000 : false,
    enabled: !lightSetupActive && (step === 'wallet' || step === 'daemon' || step === 'welcome'),
  });
  const setupCoinProfile = useSetupHubProfile(coin, step !== 'hub' && step !== 'advanced');
  ...
  const lightWalletFlow =
    setupWalletMode === 'light' || (isLight && !setupCoinPresence.hasFullNodeWallet);
  ...
  const hasFullNodeWallet = lightSetupActive
    ...
    : walletFile.data?.exists === true || walletFile.data?.legacy_wallet_detected === true;
  const showFullNodeMigrationHint =
    lightSetupActive && hasFullNodeWallet && !setupCoinHasLightWallet;

  useEffect(() => {
    if (step !== 'wallet' || !lightWalletFlow || setupCoinKeystoreUnreadable) return;
    if (!showFullNodeMigrationHint || walletAction !== 'choose') return;
    setWalletAction('restore_phrase');
  }, [step, lightWalletFlow, setupCoinKeystoreUnreadable, showFullNodeMigrationHint, walletAction]);
  ...
  }, [walletFile.data, hasFullNodeWallet, setupCoinHasLightWallet, setupWalletMode, isLight]);
  ...
  const walletUnlockAwaitingNode =
    walletAction === 'unlock' && walletSetupMode === 'loading' && walletFile.data?.exists === true;
  ...
  const advanceAfterChainWalletReady = () => {
    const twoFaEnabled =
      queryClient.getQueryData<Awaited<ReturnType<typeof twoFactorStatus>>>(['two-factor'])
  ...
  const saveConfig = useMutation({
    mutationFn: (partial: Parameters<typeof rpcSetConfig>[1]) => rpcSetConfig(coin, partial),
  });
  ...
  const openDashboardFromHub = async () => {
    ...
    for (const targetCoin of enabledCoins) {
      ...
      if (openMode) {
        ...
        if (LIGHT_WALLET_ENABLED) {
          await walletModeSetForCoin(targetCoin, openMode).catch(() => undefined);
        }
      }
    }
    await updatePrefs({
    ...
    >
      ...
      >
        ...
        {step === 'hub' && !mobileOnly && (
          <SetupWalletHub
            walletMode={setupWalletMode}
            onWalletModeChange={(mode) => void handleSetupWalletModeChange(mode)}
        ...
        {step === 'welcome' && (
          <div className="flex flex-col gap-4 text-sm text-fg-muted">
            ...
            {coin === 'vericoin' && setupWalletMode !== 'light' && (
              <p>
                New to Vericoin? Choose <strong>Create new wallet</strong> on the next steps. We
                only reuse an older wallet when one is already on this computer.
              </p>
            )}
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {mobileOnly || (setupWalletMode === 'light' && LIGHT_WALLET_ENABLED) ? (
                <>
                  <FeatureTile
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {!mobileOnly && (
                <Button size="sm" variant="ghost" onClick={() => setStep('advanced')}>
                  Advanced setup
                </Button>
              )}
              <Button size="sm" variant="ghost" disabled={onboardingBusy} onClick={goToHub}>
                Back to wallet menu
              </Button>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 text-sm">
            <div className="rounded-md border border-border bg-bg-subtle p-3">
              ...
              <p className="mt-2 text-xs text-fg-muted">
                The wallet starts your node automatically. This may take a minute while the
                blockchain index loads.
              </p>
            ...
            {runtime.data?.datadir_locked && (
              <div className="rounded-md border border-danger/30 bg-danger/10 px-3 py-2 text-xs text-danger">
                Another Verium instance is using this data directory. Quit Verium-Qt or any other
                node using the same folder, then reopen the wallet.
              </div>
            )}

            {!binary.data?.manageable && !binary.isLoading && (
              <div className="rounded-md border border-danger/30 bg-danger/10 px-3 py-2 text-xs text-danger">
                Node software is not available in this install. Re-download from the official
                release page.
              </div>
            )}

            <div className="flex flex-wrap gap-2">
              {connected && <Button onClick={() => setStep('wallet')}>Continue</Button>}
              <Button size="sm" variant="ghost" onClick={() => setStep('welcome')}>
                Back
              </Button>
              <Button size="sm" variant="ghost" disabled={onboardingBusy} onClick={goToHub}>
                Wallet menu
        ...
        {step === 'wallet' && lightWalletFlow && (
          <div className="flex flex-col gap-4">
            {setupCoinKeystoreUnreadable ? (
              <>
                <div className="rounded-md border border-warning/40 bg-warning/10 px-3 py-3 text-xs text-fg-muted">
                  <p className="font-medium text-fg">Recovery phrase required</p>
                  <p className="mt-1">
                    {mobileOnly
              ...
              <>
                <div className="rounded-md border border-border bg-bg-subtle p-3 text-xs text-fg-muted">
                  <p>
                    Light wallet — your keys are encrypted on this device. Balance and history come
                    from Vericonomy Electrum servers.
                  </p>
                </div>
                {showFullNodeMigrationHint && walletAction === 'choose' && (
                  <div className="rounded-md border border-warning/40 bg-warning/10 px-3 py-3 text-xs text-fg-muted">
                    ...
                    <Button size="sm" className="mt-3" onClick={() => void switchToFullNodeSetup()}>
                      {lightWalletCopy.setupUseFullNodeCta}
                    </Button>
                  </div>
                )}
                {walletAction === 'choose' && (
                  <div className="flex flex-col gap-3">
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <Button onClick={() => setWalletAction('create')}>Create new wallet</Button>
                      <Button variant="secondary" onClick={() => setWalletAction('restore_phrase')}>
                    ...
                    {showFullNodeMigrationHint && !mobileOnly && (
                      <p className="text-xs text-fg-subtle">{lightWalletCopy.setupCreateNewWarn}</p>
                    )}
                  </div>
                )}
                {walletAction === 'create' && (
                  ...
                  <div className="flex flex-col gap-3">
                    {!setupCoinHasLightWallet && (
                      <p className="rounded-md border border-warning/40 bg-warning/10 px-3 py-2 text-xs text-fg-muted">
                        No light wallet was found for {profile.symbol} on this device. Choose{' '}
                        <strong>Create new wallet</strong> or{' '}
        ...
        {step === 'wallet' && !lightWalletFlow && !legacyWizardActive && (
          <div className="flex flex-col gap-4">
            <div className="rounded-md border border-border bg-bg-subtle p-3 text-xs text-fg-muted">
              {walletFile.data?.path && (
                <div className="break-all text-[11px]">{walletFile.data.path}</div>
              )}
              <p className="mt-1">{walletSetupModeLabel(coin, walletSetupMode)}</p>
              {walletInfo.data && walletSetupMode === 'needs_unlock' && (
                <p className="mt-1 text-fg">
                  Balance: {walletInfo.data.balance.toFixed(8)} {profile.symbol}
                  {walletInfo.data.txcount > 0 ? ` · ${walletInfo.data.txcount} transactions` : ''}
                </p>
              )}
              {walletFile.data?.legacy_wallet_detected && walletFile.data.legacy_wallet_path && (
                <p className="mt-2 rounded-md border border-accent/30 bg-accent/5 px-2 py-1.5 text-xs text-fg-muted">
                  ...
                  . Import it, or in Advanced setup set your data directory to that folder.
                  Otherwise create a new wallet for a fresh start.
                </p>
              )}
              {walletFile.data?.is_new_install && (
                <p className="mt-2 text-xs text-fg-muted">
                  No existing {profile.displayName} wallet was detected on this Mac — you are
                  setting up a new {profile.symbol} wallet.
                </p>
              )}
            </div>
            ...
            {walletSetupMode === 'offline' && (
              <div className="rounded-md border border-warning/30 bg-warning/10 px-3 py-2 text-xs text-fg-muted">
                Go back to <strong>Start node</strong> and connect to {profile.binaryName} before
              ...
              walletAction === 'unlock' && (
                <>
                  ...
                  <div className="border-t border-border pt-3">
                    <Button size="sm" variant="secondary" onClick={() => setWalletAction('import')}>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                >
                  ...
                  <span className="text-xs text-fg-muted">
                    First time on this machine — choose a passphrase and encrypt a fresh wallet.dat.
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="text-xs text-fg-muted">
                    Restore a backup from {profile.displayName}-Qt, this app, or another computer.
                  </span>
                </button>
                <button
                  type="button"
        ...
        {step === 'recovery' && (
          <div className="flex flex-col gap-4">
            ...
            <p className="text-sm text-fg-muted">
              This phrase is separate from your wallet passphrase: it is the HD master key for your
              addresses. Save it on paper so you can restore on a new device if you lose your
              passphrase, computer, or <span className="font-mono text-xs">wallet.dat</span>.
            ...
            {recoveryError && <p className="text-xs text-danger">{recoveryError}</p>}
        ...
        {step === 'twofa' && (
          <div className="flex flex-col gap-4">
            ...
            <p className="text-sm text-fg-muted">
              Protect sends, passphrase changes, and sensitive actions with a code from an
              authenticator app. This applies to{' '}
              <strong className="font-medium text-fg">both Verium and Vericoin</strong>— your{' '}
              {profile.symbol} passphrase and recovery phrase remain separate per chain.
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {twoFa.data?.enabled && (
                <Button size="sm" onClick={() => setStep(lightSetupActive ? 'done' : 'bootstrap')}>
                  Continue
                </Button>
              )}
              <Button size="sm" variant="ghost" disabled={onboardingBusy} onClick={goToHub}>
        ...
        {step === 'bootstrap' && !lightSetupActive && isTestNetwork && (
          <div className="flex flex-col gap-3 text-sm text-fg-muted">
            ...
            <p>
              You are on the isolated Binary Chain (DACE) test network. There is no canonical
              snapshot CDN — the chain starts at genesis and grows as you mine VRM / stake VRC
              locally.
            </p>
            <div className="flex flex-wrap gap-2">
        ...
        {step === 'bootstrap' && !lightSetupActive && !isTestNetwork && (
          <div className="flex flex-col gap-3 text-sm text-fg-muted">
            ...
            <p>
              Fresh installs sync much faster from the official chain snapshot than over P2P. The
              daemon downloads, extracts, and restarts automatically.
            </p>
            <div className="flex flex-wrap gap-2">
        ...
        {step === 'done' && (
          <div className="flex flex-col gap-3 text-sm">
            ...
            <p className="text-fg-muted">
              You're ready to go. Settings, wallet backup, and advanced options are available from
              the sidebar.
            </p>
            <div className="flex flex-wrap gap-2">
        ...
        {step === 'advanced' && !mobileOnly && (
          <div className="flex flex-col gap-4 text-sm">
            <p className="text-fg-muted">
              Point the app at an existing data directory or remote node. Most users should use the
              simple flow.
            </p>
