...
export function useMiningPollCoordinator(): void {
  ...
  useQuery({
    ...
    refetchInterval: (query) => {
      ...
      return query.state.data?.running ? POOL_MINER_STATUS_POLL_MS : POOL_MINER_IDLE_PROBE_MS;
    },
    staleTime: 10_000,
    gcTime: 30_000,
  });
  ...
  const miner = useQuery({
    ...
    refetchInterval: (query) => {
      if (!visible) return false;
      return query.state.data?.active ? MINER_STATE_POLL_MS : MINER_STATE_IDLE_PROBE_MS;
    },
    staleTime: 10_000,
    gcTime: 30_000,
  });
