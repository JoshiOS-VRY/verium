...
const MANIFEST_PATH = path.join(ROOT, 'src', 'lib', 'releases-manifest.json');
...
function detectTargetTriple() {
  if (process.env.VERIUMD_TARGET_TRIPLE) return process.env.VERIUMD_TARGET_TRIPLE;
  ...
  switch (`${process.platform}-${process.arch}`) {
    ...
    default:
      throw new Error(`Unsupported host: ${process.platform}-${process.arch}; pass --triple=...`);
  }
}
...
function archiveUrlsFor(triple, version) {
  ...
  const baseRoot =
    process.env.VERIUMD_DOWNLOAD_BASE || 'https://files.vericonomy.com/vrm/releases/';
...
async function fetchToBuffer(url, redirectsLeft = 5) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(url, (res) => {
      ...
      if (status !== 200) {
        return reject(new Error(`HTTP ${status} fetching ${url}: ${res.statusMessage}`));
      }
      const chunks = [];
...
async function main() {
  ...
  if (fs.existsSync(dest) && process.env.VERIUMD_FORCE !== '1') {
    if (isStubSidecar(dest)) {
      ...
      if (sidecarArchMismatch(dest, triple)) {
        log(`Sidecar at ${dest} is Intel (x86_64) but target is ${triple}; re-fetching.`);
      } else {
  ...
  const preferMonorepo =
    ...
    (process.env.DACE_DEV === '1' || args['dace-dev'] || process.env.VERIUMD_USE_MONOREPO !== '0');
  if (preferMonorepo) {
...
main().catch((e) => {
  ...
  if (process.env.VERIUMD_OPTIONAL === '1') {
    process.stderr.write('[fetch-veriumd] VERIUMD_OPTIONAL=1 set, continuing without sidecar.\n');
    process.exit(0);
  }
  process.exit(1);
});
